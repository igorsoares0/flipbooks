...
- **Files:** storage stays private, and readers get short-lived signed URLs after the access check. Signed URLs (`backgroundImageUrl`, `imageUrl`) are never stored. Browsers upload to `incoming/{key}` (`presignUpload`); the server checks the file, then moves that exact version into place with `promoteUpload`. Never presign a PUT to a final key: the URL stays valid after the check.
- **Client IP:** rate limits key on `clientIp()` (`src/lib/rate-limit.ts`), which trusts only the proxy header named in `CLIENT_IP_HEADER`. Don't read `x-forwarded-for`, `cf-connecting-ip` and the like directly.
